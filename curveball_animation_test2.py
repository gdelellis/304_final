...
line_y, = ax.plot(xpoints_y,ypoints,label = "y")
line_z, = ax.plot(xpoints_z,zpoints,label = "z")


def initial():  #initializing objects
    line_y.set_data(xpoints_y,ypoints)  #making initial of y line is two empty lists
    line_z.set_data(xpoints_z,zpoints)   #making initial of z line is two empty lists
...
def animate(i):
    ...
    xpoints_z = []
    zpoints = []
    for t in tpoints[:i]:  #beginning to fill each empty list with their respective points up to the current frame
        #y vs. x curveball
    ...
    #setting new values for line_y and line_z after running through all frames
    line_y.set_data(xpoints_y,ypoints)
    line_z.set_data(xpoints_z,zpoints)
...
graph=FuncAnimation(fig,animate,init_func=initial,frames=len(tpoints),interval=.0001)


graph.save('curveball_animate.gif',fps=100000)
